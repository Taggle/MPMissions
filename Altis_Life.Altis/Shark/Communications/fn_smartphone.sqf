...
_cMessageHeader ctrlSetText format["Message De:                 Message:"];
ctrlEnable[887892,false];
switch(_type) do{	case 0:	{		lbClear _cPlayerList;
		{			if(alive _x && _x != player) then			{				switch(side _x) do				{					case west: {_type = "Cop"};
...
[getPlayerUID player, player] remoteExec ["TON_fnc_msgRequest",2];
	...
	case 2:	{		ctrlEnable[887892,true];
		...
		case 4:	{		createDialog "Life_smartphone_schreiben";
		ctrlSetText[88886, format["Message: %1",name life_smartphoneTarget]];
		if((FETCH_CONST(life_adminlevel) < 1)) then		{			ctrlShow[888897,false];
		};
	};
};
