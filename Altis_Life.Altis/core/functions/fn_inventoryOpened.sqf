...
if(EQUAL(_isPack,1)) exitWith {
	[localize "STR_MISC_Backpack", false] spawn quickNotif;
	true;
};
...
if(KINDOF_ARRAY(_container,_list)) exitWith {
	if(!(_container in life_vehicles) && (_container GVAR ["locked",true])) exitWith {
		[localize "STR_MISC_VehInventory", false] spawn quickNotif;
		true;
	};
};
...
if(_container isKindOf "Man" && !alive _container) exitWith {
	[localize "STR_NOTF_NoLootingPerson", false] spawn quickNotif;
	true;
};
